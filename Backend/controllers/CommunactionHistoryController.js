...
INNER JOIN Employeer_List e ON e.id = co.employeer_id WHERE co.is_deleted = 0`
    ...
    {
       return res.status(500).json({status:'false', error: 'Database error'+  error.message });
    }
}
...
const createhistory = async (req,res) => {
    ...
    const {customer_id,employeer_id,communaction_type,details,timestamp} = req.body;
...
const deletehistorybyid = async ( req, res ) =>
{
    ...
    try {
        ...
        if(result.rowsAffected[0]>0) {
            return res.status(200).json({message: 'History is deleted', status:'true'})
        }
        else {
...
module.exports={listallhistory,listallhistorybyid,deletehistorybyid,createhistory }